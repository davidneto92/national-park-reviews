...
feature "park index page paginates listings" do
  scenario "index shows first 5 parks alphabetically" do
  ...
  scenario "index shows remaining parks on subsequent pages, alphabetically" do
    ...
    expect(page).to_not have_xpath("//div[.=index-display]/..", text: "#{park_01.name}")
    expect(page).to_not have_xpath("//div[.=index-display]/..", text: "#{park_02.name}")
